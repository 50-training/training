...
public class ProductService : IProductService
{
    ...
    {
        IReadOnlyList<Product> products = await _productRepository.GetLowStockAsync(threshold);
        if (products.Count == 0)
        {
        ...
        IReadOnlyList<ProductSales> sales = await _orderRepository.GetUnitsSoldByProductSinceAsync(since, ids);
        Dictionary<int, int> soldById = sales.ToDictionary(s => s.ProductId, s => s.UnitsSold);
        ...
        return products
            .Select(p => new LowStockItem(p.Id, p.Sku, p.Name, p.StockQuantity,
                soldById.TryGetValue(p.Id, out int q) ? q : 0))
